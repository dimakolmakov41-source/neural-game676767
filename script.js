...
function applyBanState() {
    ...
    if (banEnd && Date.now() < banEnd) {
        ...
        showToast(`🌚 Ты забанен на ${Math.ceil(remaining / 60)} минут за плохие слова`);
...
let passEndDate = new Date(2026, 8, 21, 23, 59, 59);
...
function renderPassBadges() {
    ...
    document.getElementById('passLevels').innerHTML = html;
    const claimed = passTasks.filter(t => t.claimed).length;
    const percent = (claimed / passTasks.length) * 100;
    document.getElementById('passProgressFill').style.width = percent + '%';
}
...
function startPassRewardTimer() {
    ...
    passRewardInterval = setInterval(() => {
        ...
        if (passRewardSeconds <= 0) {
            ...
            const rewardDiamonds = 5;
            points += rewardPoints;
            diamonds += rewardDiamonds;
            totalDiamondsEarned += rewardDiamonds;
            updateUI();
            saveGame();
            showToast(`🎁 Награда AI Pass! +${rewardPoints}🧠 +${rewardDiamonds}💎`);
            playBuySound();
...
function updatePassRewardTimerDisplay() {
    ...
    document.getElementById('passTimer').innerHTML = `⏱️ ${m}:${s.toString().padStart(2, '0')}`;
}
...
function claimTask(idx) {
    ...
    renderTasksList();
    saveGame();
    showToast(`🎉 Уровень ${task.level} получен! +${task.rewardPoints}🧠 +${task.rewardDiamonds}💎`);
    playBuySound();
}

function checkPassProgress() {
    if (document.getElementById('passTasksModal').classList.contains('show')) {
        renderTasksList();
    }
}
...
function updatePassEndTimer() {
    ...
    if (diff <= 0) {
        document.getElementById('passEndTimer').innerHTML = "⏳ AI PASS 5 ЗАВЕРШЁН!";
    ...
    document.getElementById('passEndTimer').innerHTML = `⏳ До конца AI Pass 5: ${days} дн. ${hours} ч. ${minutes} мин.`;
}
...
function initMusic() { if (audioCtx) return; audioCtx = new (window.AudioContext || window.webkitAudioContext)(); }
...
function updatePlaytime() {
    ...
    document.getElementById('playtime').innerText = minutes;
}
...
📅 Версия ${GAME_VERSION}`;
    ...
    showToast("✅ Прогресс скопирован в буфер обмена!");
}
...
function updateUI() {
    ...
    document.getElementById('power').innerText = getClickPower();
        ...
        try {
            ...
            if (d.upgrades) {
                d.upgrades.forEach((data, i) => {
                    if (upgrades[i]) {
                        upgrades[i].purchased = data.purchased;
                    }
                });
            }
            if (d.passTasks) {
...
function importProgress(file) {
    ...
    reader.onload = function(e) {
        try {
            ...
            showToast("📂 Прогресс загружен! Перезагрузи страницу.");
...
const promoCodes = {};
for (let i = 1; i <= 100; i++) {
    promoCodes[`code${i}`] = { points: 100 + i * 5, diamonds: 1 + Math.floor(i / 10) };
}
...
setInterval(() => {
    if (reloadSeconds > 0) {
        ...
        document.getElementById('reloadCountdown').innerText = `${m}:${s.toString().padStart(2, '0')}`;
        if (reloadSeconds === 0) {
